...
# """
    ...
    source_plate = ctx.load_labware('opentrons_96_wellplate_200ul_pcr_full_skirt', 3) # do I change this to their plate?

    tiprack_50 = [ctx.load_labware('opentrons_flex_96_tiprack_50ul',  slot) for slot in [8, 9]]

    # LOAD PIPETTES
    p50 = ctx.load_instrument(
        "flex_1channel_50", p50_mount, tip_racks=tiprack_50)
    ...
    open_close_count = tc_mod._open_close_count
    print("---------open close count-----------")
    for k,v in open_close_count.items():
        print(k, v)
    ...
    with open(csv_file, 'w', newline='') as file:
        ...
        for key, value in open_close_count.items():
