...
def run(protocol: protocol_api.ProtocolContext):
    ...
    reservoir           = protocol.load_labware('nest_96_wellplate_2ml_deep','2', "Deep Well Reservoir")    
    temp_block          = protocol.load_module('temperature module gen2', '3')
    reagent_plate       = temp_block.load_labware('nest_96_wellplate_100ul_pcr_full_skirt', "Reagent Plate")
    # ========== SECOND ROW ==========
    ...
    sample_plate_1      = thermocycler.load_labware('nest_96_wellplate_100ul_pcr_full_skirt', "Sample Plate")
...
############################################################################################################################################
    ...
    for loop in range(RUN):
        ...
        if STEP_HYB == 1:
            ...
            for loop, X in enumerate(column_1_list):
                p50.pick_up_tip()
                p50.aspirate(NHB2Vol, NHB2.bottom(z=0.5)) #original = ()
                p50.dispense(NHB2Vol, sample_plate_1[X].bottom(z=0.5)) #original = ()
            ...
            for loop, X in enumerate(column_1_list):
                p50.pick_up_tip()
                p50.aspirate(PanelVol, Panel.bottom(z=0.5)) #original = ()
                p50.dispense(PanelVol, sample_plate_1[X].bottom(z=0.5)) #original = ()
                p50.return_tip() if TIP_TRASH == False else p50.drop_tip()
                p50_tips += 1
                tipcheck()
            ...
            for loop, X in enumerate(column_1_list):
                p1000.pick_up_tip()
                p1000.aspirate(EHB2Vol, EHB2.bottom(z=0.5)) #original = ()
                p1000.dispense(EHB2Vol, sample_plate_1[X].bottom(z=0.5))  #original = ()
        ...
        if STEP_CAPTURE == 1:
            ...
            for wash in range(washreps):
                ...
                for loop, X in enumerate(column_2_list):
                    p1000.pick_up_tip()
                    p1000.aspirate(EEWVol, WASHES[loop].bottom(z=0.5))  #original = ()
                    p1000.dispense(EEWVol, sample_plate_2[X].bottom(z=0.5))  #original = ()
                    p1000.return_tip() if TIP_TRASH == False else p1000.drop_tip()
                    p200_tips += 1
                    tipcheck()
                heatershaker.close_labware_latch()
            ...
            for loop, X in enumerate(column_3_list):
                p50.pick_up_tip()
                p50.move_to(sample_plate_2[X].bottom(z=0.5))  #original = z=0
            ...
            for loop, X in enumerate(column_3_list):
                p50.pick_up_tip()
                p50.aspirate(EluteVol, Elute.bottom(z=0.5))  #original = ()
                p50.dispense(EluteVol, sample_plate_2[X].bottom(z=0.5))  #original = ()
                p50.return_tip() if TIP_TRASH == False else p50.drop_tip()
                p50_tips += 1
                tipcheck()
            ...
            for loop, X in enumerate(column_4_list):
                p50.pick_up_tip()
                p50.aspirate(ET2Vol, ET2.bottom(z=0.5))  #original = ()
                p50.dispense(ET2Vol, sample_plate_1[X].bottom(z=0.5))  #original = ()
        ...
        if STEP_PCR == 1:
            ...
            for loop, X in enumerate(column_4_list):
                p50.pick_up_tip()
                p50.aspirate(PPCVol, PPC.bottom(z=0.5))  #original = ()
                p50.dispense(PPCVol, sample_plate_1[X].bottom(z=0.5))  #original = ()
                p50.return_tip() if TIP_TRASH == False else p50.drop_tip()
                p50_tips += 1
                tipcheck()
            ...
            for loop, X in enumerate(column_4_list):
                p50.pick_up_tip()
                p50.aspirate(EPMVol, EPM.bottom(z=0.5))  #original = ()
                p50.dispense(EPMVol, sample_plate_1[X].bottom(z=0.5))  #original = ()
                p50.move_to(sample_plate_1[X].bottom(z=0.5))  #original = ()
        ...
        if STEP_CLEANUP == 1:
            ...
            for loop, X in enumerate(column_5_list):
                p1000.pick_up_tip()
                p1000.move_to(sample_plate_2[X].bottom(z=0.5))  #original = (z=0)
    ...
    csv_file = 'ABR-End-Volumes/Illumina-DNA-Enrichment End Vol.csv'
